...
class Prefetcher {
 public:
  ...
  void updatePriority(const torch::Tensor& priority) {
    ...
    assert(std::abs((int)sampledIndices_.size() - (int) sampleBuffer_.size()) < 3);
 ...
 private:
  ...
  void wait_to_fill_buffer() {
    ...
    while ((size_t)sampleBuffer_.size() > bufferSize_ || (size_t)replayer_->size() < 2 * batchsize_) {
        if (done_) break;
  ...
  void wait_for_buffer_to_fill() {
    if ((size_t)replayer_->size() >= 2 * batchsize_) cv_fill_buffer.notify_one();
    std::unique_lock<std::mutex> lg (m_buffer);
